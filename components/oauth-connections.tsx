...
export function OAuthConnections({ userId }: OAuthConnectionsProps) {
  ...
  const loadConnections = useCallback(async () => {
    try {
      const { data, error } = await authService.getUserOAuthConnections(userId)
      if (error) {
  ...
  const handleDisconnect = async (provider: string) => {
    ...
    try {
      const { error } = await authService.disconnectOAuthProvider(userId, provider)
      if (error) {
  ...
  const availableProviders: ('google' | 'github')[] = ['google', 'github']
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Connected Accounts</CardTitle>
        <CardDescription>
          Manage your social login connections. You can use these to sign in to your account.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
            ...
            {connections.map((connection) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  {ProviderIcons[connection.provider as keyof typeof ProviderIcons]}
                  <div>
        ...
        {availableProviders.some(provider => !connectedProviders.has(provider)) && (
          <div className="space-y-3">
            <h4 className="text-sm font-medium">Available to Connect</h4>
            {availableProviders
        ...
        {connections.length === 0 && (
          <div className="text-center py-6 text-muted-foreground">
            ...
            <p className="text-sm">Connect a social account for easier sign-in.</p>
          </div>
        )}
      </CardContent>
    </Card>
